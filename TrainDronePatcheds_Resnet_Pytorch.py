...
# By Alfonso Blanco García, December 2024
...
train_transforms = transforms.Compose([transforms.Resize((DimPatch,DimPatch)),
...
test_transforms = transforms.Compose([transforms.Resize((DimPatch,DimPatch)),
                                      transforms.CenterCrop(DimPatch),
                                      transforms.ToTensor(),
                                      transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])

validation_transforms = transforms.Compose([transforms.Resize((DimPatch,DimPatch)),
                                            transforms.CenterCrop(DimPatch),
                                            transforms.ToTensor(),
                                            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
test_data = datasets.ImageFolder( 'valid', transform=test_transforms)
    ...
    for ii, (inputs, labels) in enumerate(trainloader):
        ...
        if steps % print_every == 0:
            ...
            checkpoint = {'state_dict': model.state_dict(),
              ...
              'num_epochs': epochs}
            # To evaluate acording training runs
            torch.save(checkpoint, 'checkpoint1epoch2Classes.pth')
...
torch.save(checkpoint, 'checkpoint10epoch2Classes.pth')
